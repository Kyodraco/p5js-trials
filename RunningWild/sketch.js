...
function setup() {
  ...
  attractors.push(createVector(0, 0, 0));
...
function initAttractors(nbAttr) {
  ...
  for (var a = 0; a < nbAttr; a++) {
    ...
    var newX = radius * cos(angle);
    var newY = radius * sin(angle);
    var newZ = radius * cos(angle) * sin(angle);
    attractors.push(createVector(newX, newY, newZ));
  }
  nbAttractors = nbAttr;
}
  ...
  switch (lyric.partType) {
    ...
    case "refrain":
      ...
      for (var p = 0; p < 10; p++) {
        ...
        particles.push(new Particle(random(width), random(height), random(width), pColor));
      }
      break;
    case "pre-pont":
      if (action != "pre-pont") {
      ...
      var newY = height / 2 + (height / 4) * cos(counter / 50) * sin(counter / 50);
      var newZ = 0;
      attractors = [createVector(newX, newY, newZ)];
      break;
    case "pont":
      setAttractorsOnText(lyric.text);
      if (action != "pont") {
    ...
    case "end-pont":
      ...
      var newX = width / 2 + (width / 4) * cos((PI / 4) * Math.floor(counter / 30));
      var newY = height / 2;
      var newZ = 0;
      attractors = [createVector(newX, newY, newZ)];
      break;
    ...
    if (this.song.isPlaying()) {
      ...
      if (showAttractors) {
        for (var a = 0; a < attractors.length; a++) {
          ...
          sphere(50, attractors[a].x, attractors[a].y)
        }
      }
/* 
